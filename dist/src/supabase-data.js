(function () {
  ...
  async function saveAdminLocation(court) {
    ...
    const payload = {
      ...
      website_url: court.websiteUrl || null,
      phone_number: court.phoneNumber || null,
  ...
  async function savePendingInstagramPostImages(locationId, imageUrls = [], actorId = null) {
    const supabase = client();
    if (!supabase || !locationId) throw new Error('Supabase is not configured.');

    const nextImageUrls = uniqueInstagramImageUrls(imageUrls);
    if (!nextImageUrls.length) throw new Error('Add at least one public JPEG image URL.');
    ...
    if (existing?.status === 'failed') payload.status = 'pending';
